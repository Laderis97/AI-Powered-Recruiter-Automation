...
export class PredictiveAnalyticsService {
  ...
  predictCandidateSuccess(
    ...
    const experienceAdjustment = this.calculateExperienceAdjustment(
      candidate,
      job
    );
  ...
  private calculateExperienceAdjustment(candidate: any, job: any): number {
    const candidateYears = this.extractYearsFromExperience(
      candidate.experience
    );
  ...
  private calculateMarketAdjustment(marketData: MarketIntelligence): number {
    ...
    const highDemandSkills = marketData.skillsDemand.filter(
      s => s.demand === 'high'
    ).length;
    const risingSkills = marketData.skillsDemand.filter(
      s => s.trend === 'rising'
    ).length;
  ...
  private predictTimeToHire(
    ...
    // Adjust based on market conditions
    const marketDemand = marketData.skillsDemand.filter(
      s => s.demand === 'high'
    ).length;
    ...
    if (
      job.parsedData?.seniority === 'Senior' ||
      job.parsedData?.seniority === 'Lead'
    ) {
      baseTime += 10;
    }
  ...
  private assessRetentionRisk(
    ...
    if (candidate.jobHistory && candidate.jobHistory.length > 0) {
      const avgTenure =
        candidate.jobHistory.reduce(
          (sum: number, job: any) => sum + (job.duration || 12),
          0
        ) / candidate.jobHistory.length;

      if (avgTenure < 12)
        riskScore += 3; // Less than 1 year average
  ...
  private calculateConfidence(candidate: any, job: any): number {
    ...
    if (job.parsedData?.skills && job.parsedData.skills.length > 5)
      confidence += 10;
  ...
  private isNearbyLocation(location1: string, location2: string): boolean {
    ...
    return (
      nearbyLocations[location1]?.includes(location2) ||
      nearbyLocations[location2]?.includes(location1) ||
      false
    );
  }

  // Market data methods
  private getBaseSalary(
    role: string,
    location: string,
    industry: string,
    seniority: string
